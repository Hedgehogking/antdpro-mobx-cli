...
export default class Creator {
  ...
  async copyTpl(tmpTarget) {
    await wrapLoading(() => {
      return new Promise((resolve, reject) => {
        let editor = memFsEditor.create(menFs.create())
        editor.copyTpl(tmpTarget, this.target, {
          projectName: this.name
        })
        editor.commit(async () => {
          // men-fs 内 ${tmpTarget}/** 匹配不了.env
          await this.exec(`cp ${tmpTarget}/.** ${this.target}`).catch(() => {})
          if (fs.existsSync(`${tmpTarget}/.env.development.local.example`)) {
            await this.exec(`cp ${tmpTarget}/.env.development.local.example ${this.target}/.env.development.local`).catch(() => {})
          }
          editor = null
          resolve()
        })
  ...
  async install() {
    await wrapLoading(() => this.exec(`cd ${this.target} && npm install`), 'Installing...', 'Install success')
  }
}
